...
- 博客「命令行」页（`/cmd`）可连接本机真实 shell：本机先 `npm run bridge`（即 `node local-bridge/bridge.js`）启动仅监听 `127.0.0.1:9876` 的 HTTP+WebSocket 服务，页面 `connect <token>`（默认 `blog-local`，启动时打印）后自动打开**持久 shell 会话**
