...
- [2026-09-03 移除命令行功能 · 导航图标](#2026-09-03-移除命令行功能--导航图标)
- [2026-09-08 内容架构收敛：8 入口 → 5 入口](#2026-09-08-内容架构收敛8-入口--5-入口)
- [2026-09-10 站点收敛：纯写作博客](#2026-09-10-站点收敛纯写作博客)
- [2026-09-10 首页改版：参考 mrzym.top 形态](#2026-09-10-首页改版参考-mrzymtop-形态)
...
## 2026-09-03 移除命令行功能 · 导航图标

### 背景

用户确认不再需要「命令行」功能（网页端连本机 shell），并要求导航链接配对应图标。

### 功能迭代清单

- **移除命令行**：删除 `src/views/CmdView.vue`（700+ 行）、`local-bridge/`（bridge.js/cmd + 日志）、router `/cmd` 路由、BlogNav cmd 导航项、package.json `bridge` script、design.css `.cmd-*` 死样式（110 行）；AGENTS.md「本地命令行桥接」整节与宪法对应行删除
- **导航图标**：BlogNav 9 个导航项（文章/随笔/视频/音乐/标签/时间线/工作台/特效/关于）+ 搜索按钮全部配单色内联 SVG（stroke 1.8、currentColor、viewBox 24）；图标数据放 items 数组 `icon` 字段，模板一处渲染，桌面/下拉菜单复用
- **图标动效**：hover/active 时图标 `translateY(-1px) scale(1.12)` 弹跳（spring 曲线），与灵动按钮体系一致

### 踩坑记录

#### 1. 设计系统「无彩色图标」约束

- 图标必须单色：SVG 用 `fill="none" stroke="currentColor"`，颜色跟随文字色（hover 变 accent 自动生效），不引入图标库（零依赖、零首屏成本）

#### 2. 删除功能要清尸体

- cmd 不只在视图层：路由、导航、script、AGENTS.md 约定、宪法、CSS 死样式都要同步删，否则留下不可达 chunk 与文档误导

### 架构要点

- 剩余 13 个路由（原 14），cmd chunk 从构建产物消失；AboutView 的「Ctrl/Cmd+K」是搜索快捷键说明，保留（非命令行功能）
...
## 2026-09-10 站点收敛：纯写作博客

### 背景

用户反馈「博客太杂、什么都有」，委托整体优化并全程自主决策，无需确认。经 speckit 规格驱动流程（specs/001-blog-simplify）执行全站收敛：从「内容 + 媒体 + 工具 + 特效」的杂烩站收敛为纯写作博客（文章/随笔/时间线/关于）。

### 功能迭代清单

- 信息架构：导航 5 项 → 4 项（文章/随笔/时间线/关于），时间线入导航；videos / music / workbench / effects / 两个编辑器共 6 条路由全部下线
- 下线模块：视频（视图/播放器/下载器/video-meta/媒体文件/download.yml 工作流）、音乐（视图/播放器/迷你播放器/音频引擎/歌词/媒体文件）、工作台 7 小工具（贪吃蛇/番茄钟/骰子/随机抽取/倒计时/密码/便签）、特效陈列室 19 种效果 + 全站粒子轨迹 + three.js、网页编辑器（文章/随笔/MarkdownEditor）、媒体管理器与背景上传、身份档案（user store）
- 保留能力：随笔回收站（删除/还原/彻底删除，githubFiles 瘦身保留 moveFile/deleteFile）、设置（外观 + 背景 URL 引用）、搜索/评论/目录/分享/字号调节、背景图 public/bg 文件保留
- 样式收敛：design.css 98.1KB → 42.2KB（444 条死规则移除；脚本按保留组件类名白名单过滤 + 运行时类补充 + 动画引用保留）
- 依赖收敛：移除 three；package.json 仅剩 vue / vue-router / marked / highlight.js
- 文档同步：README / AGENTS.md / devlog / 宪法 v2.0.0 / sitemap / index.html meta 全部更新

### 踩坑记录

1. **speckit 初始化核验**：integration/宪法/命令模板/脚本此前已就绪（2026-09-03 装入），`specify --help` 被本环境安全策略拦截（误报），改用 `specify self check` 验证 CLI 可用；`specify self check` 提示 1.0.5 可升级，保持 1.0.4 不动
2. **删除面遗漏**：首轮 git rm 后 `useThree.js` / `threeTheme.js` 仍引用已卸载的 three，靠全局 grep 补删；删除大模块后必须全仓检索残留引用再进下一步
3. **CSS 裁剪脚本误报**：类名提取会把 `:class` 表达式片段误当类名（如 `'is-sub':`），覆盖率报告缺失项多为误报；真实运行时类（flash / 时间线 kind 色 / active / on / clamp）需手工补充白名单；组合选择器（如 `.nav-link.active`）因含保留类而自动保留
4. **Edit 工具对 .github 路径偶发失败**：与 2026-09-08 轮一致，换 Write 全量重写解决

### 架构要点

- 站点定位收敛为「写作」：原创内容（文章/随笔）+ 站点足迹（时间线）+ 关于；工具/媒体/特效全部下线
- 随笔回收站 Token 改为控制台 `localStorage.setItem('notes-token', 'PAT')` 配置（编辑器下线后无设置 UI）
- 已下线模块内容全部可从 Git 历史恢复，无数据丢失

## 2026-09-10 首页改版：参考 mrzym.top 形态

### 背景

用户要求「抄 http://mrzym.top/#/home」——参考站为典型内容型个人博客（满屏插画 hero + 图文文章卡片 + 右侧网站资讯侧栏）。本站在收敛为纯写作站的基础上，按参考站形态改造首页呈现，经 speckit 规格驱动流程（specs/002-home-restyle）执行。参考站素材图与文案不搬运，hero 背景与卡片封面均用本仓库 `public/bg/` 既有素材。

### 功能迭代清单

| 功能 | 说明 |
|---|---|
| 首页 hero 横幅 | 全宽横幅：海上日落背景图 + `--hero-overlay` 纯色遮罩（亮/暗主题派生）+ 居中「记录与思考」标题/副标题/统计；复用 .hero-title/.hero-sub/.hero-stats 全局类，圆角 12px、无渐变 |
| 双栏布局 | ≤960px 单栏（侧栏置底）；左主栏：搜索 + 图文卡片流；右侧栏：博主信息 / 网站资讯（文章·随笔·标签·建站天数）/ 标签云（点击筛选）/ GitHub 源码链接 |
| 文章卡片 | 封面缩略图（168×112，缺省首字衬线占位）+ 置顶徽标 + 发表于/更新于 + 阅读时间 + 分类 chip + 标签 chips + 两行摘要；整卡可点 |
| frontmatter 新字段 | 文章支持可选 `cover` / `category` / `pinned` / `updated`；置顶优先排序（posts.js）；两篇文章已补 cover 与 category，vite-blog-note 置顶 |
| 标签筛选迁移 | 首页顶部标签 chips 移除，筛选职责移交侧栏标签云（点击滚动回列表顶部） |

### 踩坑记录

1. **Edit 工具偶发 Native execution failed**：与往轮一致（.github 路径、design.css 均触发过），design.css 改用 PowerShell `Add-Content` 追加，避免 2000 行全量重写
2. **参考站素材不可搬运**：mrzym.top 的插画/摄影图与文案（标语、公告链接）属站方资产，仅借鉴布局，封面与 hero 使用本仓库 `public/bg/` 素材；「更新于」日期不伪造，两篇文章无 updated 字段时卡片只显示发表于

### 架构要点

- 首页骨架例外：宪法 v2.0.1 起，首页允许自定义横幅 hero（复用全局类），其余页面仍强制 PageHero 骨架
- 新增派生变量 `--hero-overlay`（--bg 派生，亮 0.72 / 暗 0.7），符合「无新色值、无渐变」锁定
- 排序规则：`pinned` 优先 → `date` 倒序；excerpt 由首段去 markdown 语法自动生成（≤88 字）
